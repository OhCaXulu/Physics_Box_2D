...
update_status ModulePhysics::PostUpdate()
{
	...
	if(App->input->GetKey(SDL_SCANCODE_3) == KEY_DOWN)
	{
		...
		for (int i = 0; i < 12; i++)
		{
			b2Points[i].x = PIXEL_TO_METERS (points[j]);
			j++;
			b2Points[i].y = PIXEL_TO_METERS (points[j]);
			j++;
...
void ModulePhysics::CreateCircle(float radius,int posx, int posy)
{
	if (App->input->GetKey(SDL_SCANCODE_1) == KEY_DOWN)
	{
		b2BodyDef circle;
		circle.type = b2_dynamicBody;
		circle.position.Set(PIXEL_TO_METERS(posx), PIXEL_TO_METERS(posy));

		b2Body* b = world->CreateBody(&circle);
...
void ModulePhysics::CreateBox(uint w, uint h)
{
...
void ModulePhysics::CreateChain(int * points, const uint count)
{
	if (App->input->GetKey(SDL_SCANCODE_3) == KEY_DOWN)
	{
		//TODO 2: Create a chain shape using those vertices
